...
//var macysCategory = query.macysCategory;
...
function setupSwipe() {
	...
	window.mySwipe = Swipe(elem, {
		callback: function(index, element) {
			...
			currentProductLink = productLinks[index];
    ...
    for( var i=0; i < productArray.length ; i++ ) {
        ...
        productLink.append( $( '<div style="clear:both" ></div>' ) );        	            
    		$('#macysResults').append(productLink);
				...
				productLinks.push(product.summary.producturl);

				var productDiv = $( '<div>', { className: 'macysSwipeDiv' } );
				
		    productLink = $('<a>', { className: 'macysSwipeUrl', href: product.summary.producturl } );

        productImage = $('<img>', { draggable: 'false', width: '100%', className: 'macysSwipeImage', src: product.image[0].imageurl } );
...
function showLikeOptions() {
	window.location = currentProductLink;
}
